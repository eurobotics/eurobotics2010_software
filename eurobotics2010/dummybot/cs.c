...
//				 RIGHT_ENCODER, IMP_COEF * -1.006);
	...
	trajectory_set_speed(&mainboard.traj, SPEED_DIST_FAST*2, SPEED_ANGLE_FAST*2); /* d, a */
//	trajectory_set_speed(&mainboard.traj, SPEED_DIST_FAST, SPEED_ANGLE_FAST); /* d, a */
	...
	pid_set_gains(&mainboard.angle.pid, 500, 100, 3000); //500, 10, 7000);
	pid_set_maximums(&mainboard.angle.pid, 0, 20000, 5332); //0, 20000, 4095);
...
//	bd_set_current_thresholds(&mainboard.angle.bd, 500, 8000, 1000000, 50);
	...
	pid_set_gains(&mainboard.distance.pid, 500, 100, 3000);
	pid_set_maximums(&mainboard.distance.pid, 0, 20000, 5332); //0, 20000, 4095);
