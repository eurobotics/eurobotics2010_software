...
static volatile int32_t count1, count2;
...
{
	encoders_dspic_set_value(number, v);
...
//	PCICR  = (1<<PCIE2);
	...
	/* initialize capture modules 1 and 2 */
	IC1CONbits.ICM =0b000;		// disable Input Capture 1 module
	IC1CONbits.ICTMR = 1; 		// select Timer2 as the IC1 time base
	IC1CONbits.ICI = 0b00; 		// interrupt on every capture event
	IC1CONbits.ICM = 0b001; 	// generate capture event on every edge (rising and falling)
...
//	IC2CONbits.ICM = 0b001; 	// generate capture event on every edge (rising and falling)
//
	/* enable capture interrupts and Timer 2 */
	IPC0bits.IC1IP = 5; 	// setup IC1 interrupt priority level XXX
	IFS0bits.IC1IF = 0; 	// clear IC1 Interrupt Status Flag
	IEC0bits.IC1IE = 1; 	// enable IC1 interrupt

//	IPC1bits.IC2IP = 1; 	// setup IC2 interrupt priority level
//	IFS1bits.IC2IF = 0; 	// clear IC2 Interrupt Status Flag
//	IEC1bits.IC2IE = 1; 	// enable IC2 interrupt
//
	T2CONbits.TCKPS = 0b11;	// Timer 2 prescaler = 256, T_timer2 = 6.4 us 
...
void beacon_start(void)
{
	...
	cs_set_consign(&sensorboard.beacon.cs, 200);
}
...
//		LED4_OFF();
	...
	if (local_valid){
		...
		BEACON_DEBUG("count2-count1= %ld\t",count2-count1);		
